...
// --- 1. KONFIGURASI FIREBASE (KUNCI ASLI ANDA) ---
...
const fetchGeminiAI = async (prompt, systemInstruction) => {
  ...
  try {
    ...
    return data.candidates?.[0]?.content?.parts?.[0]?.text || "AI sedang berpikir, coba sesaat lagi.";
  } catch (error) {
...
const App = () => {
  ...
  // --- 3. KONEKSI CLOUD (FIXED) ---
  useEffect(() => {
    // Pastikan masuk ke Firebase
    signInAnonymously(auth).catch(err => {
        console.error("Gagal masuk ke Cloud:", err.code);
    ...
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      setUser(u);
      if (!u) {
          // Jika user hilang, jangan biarkan loading selamanya
  ...
  useEffect(() => {
    ...
    // Sinkronisasi Data Real-time (Firestore)
...
// --- 4. ENGINE PERHITUNGAN ---
  ...
  if (loading) return (
    <div className="h-screen flex items-center justify-center bg-slate-50 flex-col">
      ...
      <p className="text-slate-500 font-black tracking-widest">MENYAMBUNG KE EGGPRO CLOUD...</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-24 md:pb-0 md:pl-64">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-white h-screen fixed left-0 top-0 p-6 shadow-2xl z-50">
        <div className="flex items-center gap-3 mb-10 px-2">
          ...
          <span className="text-2xl font-black text-yellow-500">EGGPRO</span>
        </div>
      ...
      <main className="p-4 md:p-10 max-w-7xl mx-auto">
        {activeTab === 'dashboard' && (
          <div className="space-y-8">
            <header>
              ...
              <p className="text-slate-400 font-bold text-sm mt-1 uppercase tracking-widest">Update Otomatis dari Database</p>
            </header>
            ...
            <div className="bg-slate-900 rounded-[40px] p-8 md:p-12 text-white shadow-3xl relative overflow-hidden group">
              ...
              <div className="relative z-10 md:flex justify-between items-center gap-10">
                <div className="max-w-xl">
                  ...
                  <h3 className="text-3xl font-black mb-4 tracking-tighter">Butuh saran taktis?</h3>
                  <p className="text-slate-400 font-medium leading-relaxed">AI akan menganalisis performa kandang Anda dan memberikan solusi peningkatan HDP secara instan.</p>
                </div>
                ...
                }} className="w-full md:w-auto bg-yellow-500 text-slate-900 px-10 py-5 rounded-3xl font-black text-lg shadow-2xl hover:scale-105 transition-all flex items-center justify-center gap-3">
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 h-[450px]">
               <h3 className="font-black text-slate-400 uppercase tracking-widest text-xs mb-8">Grafik Tren Produksi Telur (Butir)</h3>
        ...
        {/* Tab Input */}
        {activeTab === 'input' && (
          <div className="max-w-2xl mx-auto bg-white p-10 md:p-16 rounded-[60px] shadow-3xl border border-slate-50">
            ...
            <form onSubmit={async (e) => {
              ...
              <button type="submit" className="w-full bg-slate-900 text-white p-6 rounded-[30px] font-black text-xl shadow-2xl hover:bg-black transition-all">
                KONFIRMASI & UNGGAH
              </button>
            </form>
          </div>
